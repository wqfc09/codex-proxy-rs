...
async fn invalid_login_json_data_should_not_echo_the_submitted_value() {
    ...
    let mut request = request(
        ...
        "/api/auth/login",
        Body::from(
            json!({ "username": "admin_1", "password": submitted, "rememberMe": true }).to_string(),
        ),
    );
    request
        .headers_mut()
...
async fn login_without_json_content_type_should_keep_415_with_an_admin_envelope() {
    ...
    let response = app(fixture.state())
        .oneshot(request(
            Method::POST,
            "/api/auth/login",
            Body::from(json!({ "username": "admin_1", "password": "secret" }).to_string()),
        ))
        .await
...
async fn admin_auth_failures_should_use_stable_chinese_contracts() {
    ...
    let mut invalid_login = request(
        Method::POST,
        "/api/auth/login",
        Body::from(json!({ "username": "admin_1", "password": "wrong-password" }).to_string()),
    );
    invalid_login
        .headers_mut()
        .insert(header::CONTENT_TYPE, "application/json".parse().unwrap());
