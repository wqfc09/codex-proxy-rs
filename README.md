...
浏览器身份统一为 Account/User，管理员和普通 User 使用同一登录页；管理员可进入管理工作区和用户工作区，普通 User 只进入自己的用户工作区。
API Key 仅作为 `/v1/*` 数据面的 Bearer 凭据，不作为浏览器登录身份。
...
1. 管理员在「账号」中添加上游账号，完成授权或导入，并按需建立账号分组
2. 在「用户」中创建或启用 User，配置并发/RPM、账号分组和套餐
3. User 登录后在「API Key」中创建自己的客户端密钥；Key 自动使用 User 的账号范围和默认上游身份，可设置 Key 级并发/RPM 与日／周二级限额
4. 打开密钥配置复制 Codex 配置，或直接导入 CCSwitch；CCSwitch 使用 `/v1/usage` 查询当前真实可用额度
